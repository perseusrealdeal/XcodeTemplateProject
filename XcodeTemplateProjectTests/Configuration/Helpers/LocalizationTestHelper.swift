...
fileprivate final class LocalizationTestHelper {
    ...
    static var currentLanguage: (langCode: String, localeCode: String)? {
        ...
        if let scriptCode = currentLocale.scriptCode { localeCode = "\(langCode)-\(scriptCode)" }
        else if let regionCode = currentLocale.regionCode { localeCode = "\(langCode)-\(regionCode)" }
